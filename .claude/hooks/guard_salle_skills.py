...
depuis n'importe quel sous-répertoire de la cible. Ce hook N'EST PAS ENCORE dans le kit
publié (`export_agentic.SOURCES` ne le liste pas, mesuré le 2026-09-02 par la revue de
commit) ; la dérivation par `__file__` est là pour le jour où il y entrera, pas la preuve
qu'il y est. `AGENT_SUPERVISION_PARTY_TOML` permet de le rediriger — même convention que
les autres journaux du dispositif, et c'est ce qui le rend testable sans toucher au réel.
...
def main() -> None:
    ...
    if not isinstance(nom, str) or "party-mode" not in nom:
        _laisser_passer()
