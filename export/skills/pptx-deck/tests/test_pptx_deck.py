...
dépendance à python-pptx/DB/HTTP.

Réalignés depuis VSCode2 `tests/test_pptx_deck.py` (arbitrage 2026-09-04, hub
réaligné sur la référence la plus avancée de la flotte) : les tests des
...
reste du fichier : aucune dépendance au domaine métier, la bibliothèque doit
rester réutilisable telle quelle par la flotte.

Run: py -m pytest .claude/skills/pptx-deck/tests/ -q (hors du sweep principal
tests/ — .claude/skills est dans norecursedirs, cf. pyproject.toml : ces
scripts sont vendorisés pour la flotte, pas du code applicatif du hub)."""
...
def test_add_forme_chevron_est_reconnu() -> None:
    """Ajouté lors du réalignement 2026-09-04 : `chevron` complète FORMES_PRST
    (exploration VSCode3 check_slide_synthese v3-refonte — marqueur de
    séquence/flux, seul preset natif sans rotation nécessaire)."""
    prs = Presentation()
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    shp = add_forme(slide, "chevron", 1, 1, 2, 1, fill="#ffffff", line="#0e2356")
    geom = shp._element.spPr.find(qn("a:prstGeom"))
    assert geom.get("prst") == "chevron"
...
# --------------------------------------------------------------------------- #
# add_quote_banner — ajouté lors du réalignement 2026-09-04 (exploration
# VSCode3 check_slide_synthese v3-refonte), absent de la version VSCode2.
# --------------------------------------------------------------------------- #


def test_add_quote_banner_pose_guillemet_point_et_texte() -> None:
    from pptx_deck import add_quote_banner

    prs = Presentation()
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_quote_banner(slide, 1, 1, 4, 0.6, "La phrase qu'on retient.")
    textes = [
        sh.text_frame.text for sh in slide.shapes if getattr(sh, "has_text_frame", False)
    ]
    assert any("La phrase qu'on retient." in t for t in textes)
    assert any("“" in t for t in textes), "guillemet décoratif absent"
    assert any(t.rstrip().endswith("•") for t in textes), "point isolé de clôture absent"
