...
_Généré le 2026-07-29 20:30 par `scripts/scan_projets.py` — ne pas éditer à la main._

## Poste de pilotage

**6 projets** · **2 en alerte** (VSCode2 🟠 majeur, VScode5 🟠 majeur) · **16 pratique(s) en écart** · **7 finding(s) ouvert(s)** · **1 run(s) à solder** · **0 retard(s) de cadence**

**À arbitrer (onglet Actions correctives)** :
- 🔴 VScode5 : 5 finding(s) ouvert(s)
- 🔴 VSCode4 : 6 pratique(s) en écart + 1 finding(s) ouvert(s)
- 🔴 VSCode2 : 1 finding(s) ouvert(s)
- 🟠 VSCode : 5 pratique(s) en écart
- 🟠 VSCode3 : 4 pratique(s) en écart
- 🟠 VSCode1 : 1 pratique(s) en écart

**Runs `en-attente-validation` à solder** (valider ou requalifier) :
- [VScode5] il y a 30 min — Corriger 1) les ecarts entre indicateurs au vert et actions correctives affichees 2) la le
...
| --- | --- | --- | --- |
| VSCode | il y a 0 min | il y a 2 j | il y a 9 h |
| VSCode1 | il y a 0 min | il y a 1 j | il y a 6 min |
| VSCode2 | il y a 0 min | il y a 9 h | il y a 3 min |
| VSCode3 | il y a 0 min | il y a 6 j | il y a 2 min |
| VSCode4 | il y a 0 min | il y a 6 j | il y a 1 min |
| VScode5 | il y a 0 min | il y a 8 h | il y a 25 min |

Veille agentic : il y a 10 h (cadence 3 j).
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| VSCode | 📊 [comop-verifie-2026-07-28.pptx](file:///C:/Users/claude.camus/Documents/VSCode/comop-pptx-prototype/output/comop-verifie-2026-07-28.pptx) | 6.10.0 (core+bmm+tea+bmb+cis) | 77 | 1 | 3 | ✅ | ✅ | PostToolUse, PreToolUse, SessionStart, UserPromptSubmit | ✅ |
...
| VScode5 | 🌐 [wiki.html](file:///C:/Users/claude.camus/Documents/VScode5 - Supervision projets/docs/wiki.html) | 6.10.0 (core+bmm) | 54 | 0 | 4 | ✅ | ✅ | PostToolUse, PreToolUse, SessionStart, UserPromptSubmit | 🟠 majeur |
...
Dernier scan superviseur local : 2026-07-29T20:30:46+02:00

**Skills utilisés** (2) : agent-supervisor (2), agent-orchestrator (1)

**Skills jamais utilisés** (75) : 71 bmad-* + deck-design-library, pptx-framed-image, revue-increment, slide-text-polish
...
Dernier scan superviseur local : 2026-07-29T20:30:47+02:00
...
**Runs d'orchestration** : 59 (partiel ×1, succes ×58)
...
Dernier scan superviseur local : 2026-07-29T20:30:49+02:00
...
Dernier scan superviseur local : 2026-07-29T20:30:50+02:00
...
Dernier scan superviseur local : 2026-07-29T20:30:51+02:00

**Skills utilisés** (7) : agent-orchestrator (88), agent-supervisor (9), audit-technique (5), veille-agentic (2), update-config (1), revue-increment (1), run (1)
...
**Runs d'orchestration** : 48 (en-attente-validation ×1, succes ×47)

**Diagnostic superviseur local (findings ouverts)** :
- p4 `pratique-design` [wiki:actions-irreversibles] — Le wiki declenche des actions couteuses ou ecrivant sur d'autres depots sans confirmation ni protection contre le double-declenchement
- p3 `pratique-design` [wiki:accessibilite-onglets] — La navigation par onglets n'a aucune semantique d'accessibilite malgre 9 panneaux et un usage quotidien
- p2 `pratique-design` [wiki:etat-porte-par-couleur-seule] — L'etat d'un projet ou d'une pratique est tantot ecrit en toutes lettres, tantot porte par la seule couleur -- deux langages visuels incoherents pour la meme notion
- p2 `pratique-design` [wiki:navigation-nommage] — Le decoupage et le nommage des 9 onglets ne guident pas le parcours de decision -- trois onglets designent le meme probleme sous trois vocabulaires sans lien croise
- p1 `pratique-design` [wiki:finitions-lisibilite] — Plusieurs finitions de lisibilite degradent la lecture sans empecher de comprendre : troncatures brutes, texte de legende utilise comme corps de texte, colonnes trop etroites
...
| 🟠 Intégration continue | Build + tests rejoués à chaque push, feedback rapide. | CI GitHub Actions détectée sur VSCode, VSCode1, VSCode2, VScode5 (4/6). | Dimension Pratiques + rules (présence .github/workflows). |
...
| 🟠 Analyse statique / linter | Style et erreurs détectés automatiquement (ruff, ESLint). | Linter détecté sur VSCode1, VSCode2, VSCode3, VSCode4, VScode5 (5/6). | Dimension Pratiques + rules (présence linter). |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| VSCode | 🟢 6 fichier(s) de test, coverage configuré | 🟢 2 test(s) à vérification réelle | 🟢 hook pré-commit, bmad-code-review | 🟢 skill + hook SessionStart | 🟠 deck-design-library, ppt-designer | 🟠 wiki, CLAUDE.md | 🟠 besoins + brief BMAD | 🟠 CI, CLAUDE.md | 🟢 .env gitigné, deny rules, guard git |
| VSCode1 | 🟢 13 fichier(s) de test, coverage configuré | 🟢 2 test(s) à vérification réelle | 🟢 agent reviewer, hook pré-commit, bmad-code-review | 🟢 skill + hook SessionStart | 🟢 deck-design-review, deck-design-library, ppt-designer | 🟢 README+usage, wiki+html, CLAUDE.md | 🟢 persona, why, besoins + brief BMAD | 🟢 linter, CI, CLAUDE.md, conventions | 🟢 deny rules, guard git |
| VSCode2 | 🟢 40 fichier(s) de test, coverage configuré | 🟢 24 test(s) à vérification réelle | 🟢 hook pré-commit, bmad-code-review | 🟢 skill + hook SessionStart | 🟢 deck-design-review, deck-design-library | 🟢 README+usage, wiki+html, CLAUDE.md | 🟢 persona, why, besoins + brief BMAD | 🟢 linter, CI, CLAUDE.md, conventions | 🟢 .env gitigné, deny rules, guard git |
| VSCode3 | 🟠 4 fichier(s) de test, pas de coverage | 🟢 2 test(s) à vérification réelle | 🟢 hook pré-commit, bmad-code-review | 🟢 skill + hook SessionStart | 🟢 deck-design-review, deck-design-library, ppt-designer | 🟠 wiki+html, CLAUDE.md | 🟢 persona, why, besoins, valeur + brief BMAD | 🟢 linter, CLAUDE.md, conventions | 🟢 deny rules, guard git |
| VSCode4 | 🟠 1 fichier(s) de test, pas de coverage | 🟠 1 test(s) à vérification réelle | 🟢 hook pré-commit, bmad-code-review | 🟢 skill + hook SessionStart | 🟢 deck-design-review, deck-design-library, ppt-designer | 🟠 wiki+html, CLAUDE.md | 🟢 persona, why, besoins, valeur + brief BMAD | 🟠 linter, CLAUDE.md | 🟢 .env gitigné, deny rules, guard git |
| VScode5 | 🟢 10 fichier(s) de test, coverage configuré | 🟢 2 test(s) à vérification réelle | 🟢 hook pré-commit, bmad-code-review | 🟢 skill + hook SessionStart | ⚪ ne produit pas de deck | 🟢 README+usage, wiki+html, CLAUDE.md | 🟢 persona, why, besoins, valeur + brief BMAD | 🟢 linter, CI, CLAUDE.md, conventions | 🟢 deny rules, guard git |
...
| --- | --- | --- | --- | --- | --- |
| VSCode | 🟢 ok | 🟢 ok | 🟠 moyen | 🟢 ok | 2026-07-23 |
...
| [Dev Browser — l'agent vérifie son travail dans un vrai navigateur (Playwright + sandbox WASM)](https://github.com/sawyerhood/dev-browser) | skill | nouveau | VSCode1, VSCode2 | Skill/plugin qui donne à Claude Code un navigateur piloté (API Playwright + outils pixel/DOM, scripts en sandbox QuickJS WASM sans accès disque/réseau hôte) pour TESTER ce qu'il produit — la vérification réelle des écrans de VSCode1 (questionnaire web) et VSCode2 (FastAPI+HTMX) repose aujourd'hui sur des tests HTTP et des screenshots manuels ; à comparer avant d'écrire un harnais navigateur maison. |
